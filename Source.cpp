...
int main(int argc, char *argv[])
{
  ...
  if (existBasis > 0)
  {
    ...
    for (int i = 0; i < existBasis; i++)
    {
      ...
      if (svm.CheckOverlap(Basis) == 1)
      {
        ...
        if (isnan(Ei) || (Ei > EE) || ((i > 20) && (Ei / EE > 1.4)))
        {
          printf("\t iter = %4d     E = %14.8f , %14.8f  suspected state, get rid of it.\n", i + 1, Ei, EE);
          Basis.pop_back();
        }
        else if (i < existBasis - 1)
          EE = Ei;
      }
      else
      {
        printf("\t iter = %4d     linearly dependent state, get rid of it.\n", i + 1);
        Basis.pop_back();
      }
      fflush(stdout);
    }
  ...
  else
  {
    ...
    {
      cout << "\t Finding new state with appropriate overlap failed" << endl
           << endl;
      return 0;
    }
    itr = 1;
  ...
  while (itr < input.maxbasis)
  {
    ...
    if (NewState[0](0, 0) == 2000)
    {
      cout << "\t Finding new state with lower energy failed" << endl
           << endl;
      return 0;
    }
    Basis.push_back(NewState);
